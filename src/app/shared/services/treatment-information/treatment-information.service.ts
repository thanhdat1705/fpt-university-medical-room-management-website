...
export class TreatmentInformationService {
  ...
  getPatientTreatmentDetails(id: any, param: any) {
    ...
    this.summaryService.getTreatmentDetails(id, param).subscribe(
      (response) => {
        this.router.navigate(['/treatment-information-management/treatment-detatils', id], {
          fragment: response.data
        });
        console.log(response.data);
      },
      (error) => {
        this.router.navigate(['/treatment-information-management/treatment-history']);
        console.log(error);
        this.generalService.createErrorNotification(error);
      }
    )
  ...
  getPatientTreatment(patientId: string) {
    ...
    this.summaryService.searchTreatment(this.treatmentSearchRequest).subscribe(
      (response) => {
        this.router.navigate(['/treatment-information-management/patient-treatment-list', patientId], {
